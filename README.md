...
| SAM 3D Objects | `text2scene.py` 单物体 RGBA -> 3DGS PLY 重建权重和 pipeline 配置 | `/models/sam-3d-objects` | 约 8G | [facebook/sam-3d-objects](https://www.modelscope.cn/models/facebook/sam-3d-objects) | [facebook/sam-3d-objects](https://huggingface.co/facebook/sam-3d-objects) |
...
| DINOv2 with registers large | SAM3D object condition embedder | `/models/dinov2-with-registers-large`，由 `DINO_MODEL` 指定 | 约 1.2G | [facebook/dinov2-with-registers-large](https://www.modelscope.cn/models/facebook/dinov2-with-registers-large) | [facebook/dinov2-with-registers-large](https://huggingface.co/facebook/dinov2-with-registers-large) |
...
| 环境 | 用途 |
|------|------|
| `hyworld2` | WorldNav、WorldStereo、WorldMirror、3DGS 训练和查看，以及 WorldNav 使用的 VLM shim。 |
| `hyworld2-pano` | HY-Pano 2.0 全景图生成。 |
...
CUDA_VISIBLE_DEVICES=0 /opt/miniconda3/bin/conda run --no-capture-output -n hyworld2 \
  python -u traj_generate.py \
    --target_path "$SCENE" \
    --llm_addr localhost --llm_port 8000 --llm_name Qwen/Qwen3.5-4B \
...
| `--profile` | `docker.py run` 参数；按 wrapper stage 输出耗时、GPU 峰值显存和平均利用率到 `examples/worldgen/<runname>/profiles/`。 |
| `--profile-interval 1.0` | `docker.py run` 参数；设置 GPU 采样间隔，单位秒。 |
