...
# Install Miniconda. The repository uses two envs: hyworld2 for
# WorldNav/WorldStereo/WorldMirror/3DGS, and hyworld2-pano for panorama
# generation plus the lightweight VLM shim.
...
alias flux2='conda activate flux2'
EOF
...
# pipeline's explicit model env vars below point at real local directories.
# /cache stays for writable runtime caches mounted from the host.
...
ENV WORLDSTEREO_BASE_MODEL=/models/Wan2.1-I2V-14B-480P-Diffusers
ENV WORLDMIRROR_MODEL=/models/HY-World-2.0
ENV MOGE_MODEL=/models/moge-2-vitl-normal
...
ENV DINO_MODEL=/workspace/hyworld2/third_party/dinov2
