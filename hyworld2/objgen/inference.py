...
class Inference:
    ...
    def __init__(self, config_file: str, compile: bool = False):
        ...
        moge_model = os.environ.get("MOGE_MODEL")
        if moge_model and "depth_model" in config:
            if os.path.isdir(moge_model):
                moge_model = os.path.join(moge_model, "model.pt")
            if "moge-2" in moge_model or "moge2" in moge_model:
                config.depth_model.model._target_ = "moge.model.v2.MoGeModel.from_pretrained"
            config.depth_model.model.pretrained_model_name_or_path = moge_model
