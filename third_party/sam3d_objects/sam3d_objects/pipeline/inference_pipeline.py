...
try:
    ...
    if exc.name not in {"kaolin", "pyvista", "pymeshfix", "xatlas"}:
        raise
...
class InferencePipeline:
    def __init__(
        ...
        with self.device:
            ...
            if "mesh" in self.decode_formats or os.environ.get("SAM3D_ENABLE_MESH_DECODER") == "1":
            ...
            if slat_decoder_mesh is not None:
                models["slat_decoder_mesh"] = slat_decoder_mesh
    ...
    def postprocess_slat_output(
        ...
        if "mesh" in outputs:
            if postprocessing_utils is None:
                raise RuntimeError(
                    "Mesh/GLB postprocessing dependencies are not installed in this image"
                )
    ...
    def decode_slat(
        ...
        with torch.no_grad():
            if "mesh" in formats:
                if "slat_decoder_mesh" not in self.models:
                    raise RuntimeError(
                        "Mesh decoder is not loaded; install mesh dependencies and set SAM3D_ENABLE_MESH_DECODER=1"
                    )
